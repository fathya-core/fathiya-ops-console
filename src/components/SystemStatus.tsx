import { Activity, Github, Workflow, Zap, Cpu, Bug, Server, Lock } from 'lucide-react';

type Status = 'active' | 'mock' | 'connected-guarded' | 'disconnected' | 'draft';
...
const items: { icon: typeof Server; name: string; sub: string; status: Status }[] = [
  ...
  { icon: Cpu, name: 'Market Intel Engine', sub: 'محرك استخبارات السوق', status: 'mock' },
  { icon: Bug, name: 'Bug Bounty Engine', sub: 'محرك مكافآت الثغرات', status: 'mock' },
  { icon: Github, name: 'GitHub', sub: 'تكامل المستودعات', status: 'disconnected' },
  { icon: Workflow, name: 'n8n', sub: 'أتمتة سير العمل', status: 'disconnected' },
...
export function SystemStatus() {
  return (
    <section className="rounded-2xl border border-gold-700/30 bg-ink-900/50 overflow-hidden">
      <div className="flex items-center justify-between px-6 py-4 border-b border-gold-700/20">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <div className="font-mono text-[10px] text-gold-600/80 tracking-wider">SYSTEM STATUS</div>
          </div>
        </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 divide-x divide-y divide-gold-700/10 [direction:ltr]">
        {items.map((it) => {
